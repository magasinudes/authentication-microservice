...
You can send to the front end with `export FRONT_END_URL="http://localhost:3000/my/dashboard/"`

## How to run
You can do `./gradlew bootRun`. That will start the Spring server. You can test with `curl localhost:5002`

## Build the docker image
You can build by running : `docker build -t justin2997/authentication-microservice .`
You can run with : `docker run -p:5002:5002 -d justin2997/authentication-microservice:latest`
